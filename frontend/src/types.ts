...
export interface TruckRequest {id:string;request_timestamp?:string;cluster:string;region:string;dock_no:string;backlogs:number;backlogs_timestamp?:string|null;ob_fte?:string|null;truck_size:string;truck_type:string;plate_number?:string|null;provide_time?:string|null;linehaul_trip_no?:string|null;docked_time?:string|null;status:Status;rejection_remarks?:string|null;driver_id?:string|null;created_by:string;created_at:string;updated_at?:string|null}
...
export interface Page<T> {data:T[];current_page?:number;last_page?:number;per_page:number;from?:number|null;to?:number|null;total?:number;next_cursor?:string|null;prev_cursor?:string|null;has_more?:boolean}

export interface RequestMetrics {total:number;awaiting_action:number;by_status:Partial<Record<Status, number>>}
export type AnalyticsRange = 'shift' | 'weekly' | 'monthly';
export interface RequestAnalytics {truck_sizes:Partial<Record<'4W'|'6W'|'10W'|'6WF',number>>;hourly:Array<{label:string;count:number}>;range:AnalyticsRange;shift_start:string}
