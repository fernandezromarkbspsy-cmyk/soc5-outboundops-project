  ...
  return <div className="workspace-view dashboard-view">
    ...
    <section className="dashboard-grid">
      <article className="panel dashboard-list-panel">
        ...
        <div className="dashboard-list">{linehaulTrips.length ? linehaulTrips.map(request => <div className="linehaul-row" key={request.id}><span className="avatar">{(request.driver_id || request.requested_by || request.created_by).slice(0, 1).toUpperCase()}</span><div><strong>{request.driver_id || 'Driver pending'}</strong><small>Doc Officer: {request.requested_by || request.created_by}</small></div><span>{request.linehaul_trip_no || 'Trip pending'}</span><span>{request.cluster}</span></div>) : <p className="compact-empty">No linehaul trips have been created yet.</p>}</div>
      </article>
