...
export function AppHeader({ user, view, onRoleChange, onSearch }: Props) {
  ...
  useEffect(()=>{if(!latestUnread){acknowledgedNotification.current=null;stopAlertSound(notificationSoundSource);return;}if(knownNotification.current===null){knownNotification.current=latestUnread.id;}else if(latestUnread.id!==knownNotification.current){knownNotification.current=latestUnread.id;acknowledgedNotification.current=null;setToast(latestUnread);window.setTimeout(()=>setToast(null),5000);void client.invalidateQueries({queryKey:['requests']});void client.invalidateQueries({queryKey:['kpi']});}if(acknowledgedNotification.current!==latestUnread.id)startAlertSound(notificationSoundSource);},[latestUnread,client]);
