...
export function RequestFilters({ filters, exporting, onChange, onExport, onRefresh, createButton }: Props) {
  ...
  return <section className="request-controls" aria-label="Request filters">
    <label className="search-field compact"><Search size={17} /><input aria-label="Search by plate number" placeholder="Search by plate #" value={filters.search} onChange={event => change({ search: event.target.value })} /></label>
    {createButton}
